...
tfrecord_file = "flowers_train.tfrecord"
...
with slim.arg_scope(resnet_arg_scope()):
    logits, end_points = resnet_v2_152(images, num_classes=classes, is_training=True)

exclude = ["resnet_v2_152/logits"]
variables_to_restore = slim.get_variables_to_restore(exclude=exclude)

learning_rate = tf.Variable(initial_value=0.00003, trainable=False, name="learning_rate", dtype=tf.float32)
...
with tf.Session() as sess:
    ...
    tf.train.Saver(variables_to_restore).restore(sess, checkpoint_file)
    ...
    dataset_train = tf.data.TFRecordDataset([tfrecord_file])
